...
class SQLQueryFilter:
    ...
    def _extract_subqueries(self, sql_query: str) -> List[str]:
        # Extracts subqueries within parentheses
        subqueries = []
        in_subquery = False
        stack = []
        current_subquery = []
        ...
        for char in sql_query:
            if char == '(':
                ...
                if stack:
                    ...
                    in_subquery = False
                    current_subquery.append(char)
                    subqueries.append(''.join(current_subquery).strip())
                    current_subquery = []
            else:
                current_subquery.append(char)
    ...
    def _extract_ctes(self, sql_query):
        # Use sqlparse to extract CTEs
        parsed = sqlparse.parse(sql_query)
        ctes = []
        for stmt in parsed:
            # Look for CTEs in the statement
            for token in stmt.tokens:
                if token.ttype is None and 'WITH' in token.value.upper():
                    # Extract the entire CTE block
                    cte_block = token.value
                    ctes.append(cte_block)
        return ctes

    def _apply_filter_recursive(self, sql_query: str, client_id: int) -> str:
        cte_texts = self._extract_ctes(sql_query)
        
        # Apply filter to each CTE
        for cte_text in cte_texts:
            filtered_cte = self._apply_filter_to_single_query(cte_text, client_id)
            sql_query = sql_query.replace(cte_text, filtered_cte)
        
        # Extract and apply filter to subqueries
        subquery_texts = self._extract_subqueries(sql_query)
        for subquery_text in subquery_texts:
            filtered_subquery = self._apply_filter_to_single_query(subquery_text, client_id)
            sql_query = sql_query.replace(subquery_text, filtered_subquery)
        
        # Apply filter to the main query
        if 'WHERE' in sql_query.upper():
            result = self._apply_filter_to_single_query(sql_query, client_id)
        else:
            result = self._inject_where_clause(sql_query, client_id)
    ...
    def _inject_where_clause(self, sql_query: str, client_id: int) -> str:
        where_clause = f'"o"."user_id" = {client_id}'
        
        # Apply filter to the main query
        if 'WHERE' in sql_query.upper():
            # Ensure proper placement of the additional condition
            sql_query = re.sub(r'WHERE\s+(.+?)(?=ORDER\s+BY|LIMIT|$)', f'WHERE \\1 AND {where_clause}', sql_query, flags=re.IGNORECASE)
        else:
